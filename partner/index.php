<?php 
    ...
    $pCourses = getByQuery(sprintf("SELECT co.id AS course_id, co.name AS course_name, co.description AS course_description, co.price AS course_price, co.picture AS course_picture, pa.name AS partner_name, pa.logo AS partner_logo, (SELECT COUNT(ad.id) FROM admissions ad WHERE ad.course_id = co.id) AS admission_count, us.name AS tutor_name FROM courses co INNER JOIN partners pa ON pa.id = co.partner_id INNER JOIN users us ON us.id = co.tutor_id WHERE pa.id = %s", $pId));
...
<head>
    ...
    <title>Eduweb | Partner - courses</title>
...
<section class="min-h-screen w-full max-w-[1280px] mx-auto h-full py-16 justify-center relative">
    ...
    <img src="../images/prpl-curl.svg" alt="decor-lamp" width="100" height="100" class="absolute top-32 right-6 rotate-180">
        <div class="space-y-4">
            <a href="../partners.php" class="space-x-2 border-b border-slate-800 font-medium"><i class="fa-solid fa-chevron-left"></i><span>Kembali</span></a>
